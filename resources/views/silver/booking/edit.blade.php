...
<section id="admin2-dashboard">
    <div class="admin2-menu">
        <ol>
            ...
            @if($platinium || $gold || $silver)
                <a href="{{route('booking', app()->getLocale())}}"><li class="p-lead active"><i class="fas fa-paste"></i> Booking</li></a>
            @endif
            @if($platinium)
    ...
    <div class="admin2-dashboard-content">
        ...
        <div class="dashboard-content">
            <select name="selectorLng" id="selectorLng" onchange="location = this.value;">
                @if($parametarExport == 'de')
                    @foreach (config('app.available_locales') as $locale)
                        <option value="{{ route(\Illuminate\Support\Facades\Route::currentRouteName(), [$locale, $booking]) }}">
                            <a class="nav-link"
                               href="{{ route(\Illuminate\Support\Facades\Route::currentRouteName(), [$locale, $booking]) }}"
            ...
            <a href="/{{$parametarExport}}/admin/booking" class="back-button">Back</a>
            {!! Form::open(['method'=>'DELETE', 'action'=>['OnlineBookingController@destroy', app()->getLocale(), $booking->id]]) !!}
